    ...
    void Draw(sf::RenderWindow& window) {
        ...
        for (int i = 0;i < size;i++) {
            for (int j = 0;j < size;j++) {
                ...
                if ((i + j) % 2 == 0) {
                    tile.setFillColor(sf::Color(171, 129, 79, 255));
                }
                else {
                    tile.setFillColor(sf::Color(101, 67, 33, 255));
                }
                window.draw(tile);
            }
        }
...
void Setup(sf::RenderWindow& window, Piece* BlackPieces, Piece* WhitePieces) {
    int m = 0;
    for (int i = 0;i < 3;i++) {
...
void KillPiece(int x, int y, Piece* BlackPieces, Piece* WhitePieces, int *turn) {
    ...
    while (window.isOpen()) {
        ...
        if (selected) {
            ...
            if (FindPiece(x, y, BlackPieces, WhitePieces) && (FindPiece(x, y, BlackPieces, WhitePieces)->color == sf::Color::Black && turn == 1 || FindPiece(x, y, BlackPieces, WhitePieces)->color == sf::Color::White && turn == 2)) {
                if (FindPiece(x, y, BlackPieces, WhitePieces) == SelectedPiece) {
                    SelectedPiece = NULL;
                }
                else {
